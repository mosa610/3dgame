...
{
	// 送るデータが存在しているか確認
	if (data.empty())return E_NOTIMPL;
	...
	// バッファの作成
	HRESULT hr = device->CreateBuffer(&bufferDesc, &subresourceData, structuredBuffer.GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	shaderResourceViewDesc.Buffer.NumElements = static_cast<UINT>(data.size());
	hr = device->CreateShaderResourceView(structuredBuffer.Get(), &shaderResourceViewDesc, shaderResourceView);
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
