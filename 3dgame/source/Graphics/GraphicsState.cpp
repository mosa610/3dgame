...
GraphicsState::~GraphicsState()
{
	for (size_t i = 0; i < static_cast<size_t>(SAMPLER_STATE::End); i++)
	{
		sampler_states[i].Reset();
	}
	for (size_t i = 0; i < static_cast<size_t>(BLEND_STATE::End); i++)
    {
        blend_states[i].Reset();
    }

    for (size_t i = 0; i < static_cast<size_t>(DEPTH_STATE::End); i++)
    {
        depth_stencil_states[i].Reset();
    }
    for (size_t i = 0; i < static_cast<size_t>(RASTER_STATE::End); i++)
    {
        rasterizer_states[i].Reset();
    }
}

void GraphicsState::Initialize(ID3D11Device* device)
{
	HRESULT hr = S_OK;
...
void GraphicsState::DeInitialize()
{
	for (size_t i = 0; i < static_cast<size_t>(SAMPLER_STATE::End); i++)
	{
		sampler_states[i].Reset();
...
void GraphicsState::SetBlendState(ID3D11DeviceContext* dc, BLEND_STATE state)
{
	dc->OMSetBlendState(blend_states[static_cast<size_t>(state)].Get(), nullptr, 0xFFFFFFFF);
}

void GraphicsState::SetDepthStencilState(ID3D11DeviceContext* dc, DEPTH_STATE state)
{
	dc->OMSetDepthStencilState(depth_stencil_states[static_cast<size_t>(state)].Get(), 0);
}

void GraphicsState::SetRasterizerState(ID3D11DeviceContext* dc, RASTER_STATE state)
{
...
void GraphicsState::SetSamplerState(ID3D11DeviceContext* dc, SAMPLER_STATE state)
{
	...
	{
		dc->PSSetSamplers(static_cast<int>(state), 1, sampler_states[static_cast<int>(state)].GetAddressOf());
	}
}
