...
framebuffer::framebuffer(ID3D11Device* device, uint32_t width, uint32_t height, bool depth_stencil)
{
    HRESULT hr{ S_OK };
...
void framebuffer::UpdateWindowSize(ID3D11Device* device, uint32_t width, uint32_t height)
{
...
void framebuffer::ResetShaderResourceView()
{
    render_target_view.Reset();
    depth_stencil_view.Reset();
    cached_depth_stencil_view.Reset();
    cached_render_target_view.Reset();
    shader_resource_view[0].Reset();
    shader_resource_view[1].Reset();
    shader_resource_view->Reset();
}

void framebuffer::RemakeShaderResourceView(ID3D11Device* device)
{
