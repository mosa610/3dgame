...
float4 sample_skybox(float3 v, float roughness)
{
    ...
    float2 sampler_point;
    sampler_point.x = (atan2(v.z, v.x) + PI) / (PI * 2.0);
    sampler_point.y = 1.0 - ((asin(v.y) + PI * 0.5) / PI);
    return skybox.SampleLevel(sampler_states[LINEAR], sampler_point, lod);
}
